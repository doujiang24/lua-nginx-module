...
=== TEST 31: default jit_stack_size too small
--- config
    location /re {
...
=== TEST 32: increase jit_stack_size
--- http_config
    init_by_lua_block {
...
=== TEST 33: jit_stack_size change disallowed once regex cache is populated
--- config
    location /re {
        content_by_lua_block {
            local s = "hello, 1234"
            ngx.re.find(s, "(hello world)|([0-9])", "jo")
...
--- request
    GET /re
--- response_body
content_by_lua(nginx.conf:45):4: Changing jit stack size is not allowed when some regexs have already been compiled and cached
--- no_error_log
[error]
